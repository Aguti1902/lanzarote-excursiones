  ...
  return (
    <div className="grid gap-8 lg:grid-cols-[minmax(0,1.35fr)_minmax(320px,0.85fr)] lg:items-start">
      <div className="space-y-8">
          ...
          {directionOptions.map((opt) => {
            ...
            return (
              <button
                ...
                className={`rounded-xl px-4 py-4 text-center text-sm font-bold transition ${
                  active
                    ? "bg-ocean text-white shadow-sm"
                    : "bg-ocean/85 text-white/95 hover:bg-ocean"
                }`}
              >
                {opt.label}
              </button>
        ...
        <ul className="grid gap-3 sm:grid-cols-2">
          {highlights.map((h) => (
            <li
              key={h}
              className="flex items-start gap-3 rounded-xl bg-white px-4 py-3.5 text-sm text-ink ring-1 ring-sand-line"
            >
        ...
        <div className="overflow-hidden rounded-xl bg-white ring-1 ring-sand-line">
            ...
            <table className="w-full min-w-[640px] text-left text-sm">
              ...
              <tbody>
                {destinations.map((d) => (
                  <tr
                    key={d.id}
                    className={`cursor-pointer border-b border-sand-line last:border-0 hover:bg-sky-soft/40 ${
                      destination === d.id ? "bg-sky-soft/50" : ""
                    }`}
          ...
          <p className="border-t border-sand-line px-4 py-2 text-xs text-ink-muted">
            Precio por vehículo · hasta {INCLUDED_PASSENGERS} pasajeros
            incluidos · IGIC incluido
          </p>
        </div>
      </div>
      ...
      <form
        ...
        className="rounded-xl bg-white p-5 shadow-sm ring-1 ring-sand-line lg:sticky lg:top-24"
      >
        ...
        <div className="mt-5 border-t border-sand-line pt-4">
          ...
          {adults > INCLUDED_PASSENGERS && (dest.priceExtraPerson ?? 0) > 0 && (
            <p className="mt-1 text-xs text-ink-muted">
              Incluye {adults - INCLUDED_PASSENGERS} persona
              {adults - INCLUDED_PASSENGERS === 1 ? "" : "s"} extra
            </p>
          )}
          <button
            type="submit"
